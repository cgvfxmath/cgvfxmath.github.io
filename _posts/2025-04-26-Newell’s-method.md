...
다음 그림과 같이 네 점 $\mathbf{P}_1$, $\mathbf{P}_2$, $\mathbf{P}_3$, $\mathbf{P}_4$로 구성되는 사각형이 있을 때, 이 사각형의 법선 벡터 $\overrightarrow{\mathbf{N}}$의 $x$, $y$, $z$ 성분인 $N_x$, $N_y$, $N_z$는 각각 오른쪽 수식과 같이 계산할 수 있다는 것이 **뉴웰 방법(Newell's method)** 방법의 내용입니다.
