...
3차원 공간에서 평평한 도형이 있을 때, 이 도형의 법선 벡터는 3개의 2차원 평면($xy$-평면, $yz$-평면, $zx$-평면)으로 투영한 그림자의 넓이와 밀접한 관련이 있습니다. 예를 들어, $xy$-평면으로 투영한 그림자의 넓이가 주어진 다각형의 넓이와 같다면, 이 도형은 $xy$-평면과 평행이므로 법선 벡터는 $xy$-평면과 수직이고, 이 법선 벡터의 $x$성분과 $y$성분의 값은 0이 되어야 하므로, $\overrightarrow{\mathbf{N}}$은 $(0,0,1)$이 됩니다.
