...
3차원 공간에서 평평한 도형이 있을 때, 이 도형의 법선 벡터는 3개의 2차원 평면($xy$-평면, $yz$-평면, $zx$-평면)으로 투영한 그림자의 넓이와 밀접한 관련이 있습니다. 예를 들어, 이 도형이 $xy$-평면과 평행이라면, 세 2차원 평면 $yz$-평면과 $zx$-평면으로 투영한 그림자의 넓이는 0이며, $xy$-평면으로 투영한 그림자의 넓이가 주어진 다각형의 넓이와 같습니다. 즉, 이 도형의 법선 벡터의 $x$성분과 $y$성분의 값은 모두 0이 되어야 합니다. 따라서 $\overrightarrow{\mathbf{N}}$은 $(0,0,1)$임을 알 수 있습니다.
