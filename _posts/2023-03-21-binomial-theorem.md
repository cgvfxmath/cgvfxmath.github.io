...
$(a+b)^{4}$를 전개하여 생성된 항들 중에서 $ab^{3}$항은 이중에서 3개의 $b$를 가지고 있는 $abbb, babb, bbab, bbba$가 합해져서 만들어진다. 즉, 4개 주머니에에서 구슬을 하나씩 꺼낼 때 순서에 상관없이 3개의 구슬 $b$가 존재하는 경우의 수이므로 이 항의 계수는 $\_{4}C\_{3}=4$가 됨을 알 수 있다.
