...
왜냐하면 $(a+b)^n$을 전개한 식의 $k+1$번째 항은 $\_{n}C\_{k} a^{n-k} b^{k}$인데, 이 항의 계수인 $\_{n}C\_{k}$는 두 개의 구슬 $a$와 $b$를 가지는 $n$개의 주머니에서 순서에 상관 없이 하나씩 꺼낼 때 $k$개의 $b$를 뽑는 것과 같은 경우의 수를 가지기 때문이다.
...
$(a+b)^{4}$를 전개하여 생성된 항들 중에서 $ab^{3}$항은 이중에서 $abbb, babb, bbab, bbba$ 이렇게 4개 항들이 합해져서 만들어진다. 즉, 4개 주머니에 있는 구슬들을 하나씩 꺼낼 때 순서에 상관없이 3개의 구슬 $b$가 존재하는 경우의 수이므로 이 항의 계수는 $\_{4}C\_{3}=4$가 됨을 알 수 있다.
