...
2. set_usd_env.bat을 실행할 때 마다 PATH와 PYTHONPATH에 필요한 경로가 계속적으로 추가되어 이 두 변수의 길이가 점점 더 길어집니다.

3. 명령 프롬프트(Command Prompt) 창을 닫으면 set_usd_env.bat을 이용하여 설정한 환경변수 내용이 모두 사라집니다.

해결 방법은 다음과 같습니다.

1. 시스템(또는 계정)의 환경변수 PYTHONPATH에 “%USD_INSTALL_DIR%\lib\python”를 추가합니다. 이것은 이 포스트의 맨 앞에서 “pip install usd-core”로 설치한 패키지가 아닌 nVIDIA USD에서 제공하는 pxr 패키지를 사용하기 위함입니다.

2. 시스템(또는 계정)의 환경변수 PATH에 %USD_INSTALL_DIR%\scripts를 추가합니다. 이것은 어느 경로에서나 set_usd_env.bat를 실행하기 위함입니다.
...
* usdview_gui.bat은 default sphere를 하나 보여주는 창이 뜨게 됩니다.

주의할 점은 보고 싶은 USD 파일을 절대경로로 지정해주어야 합니다.
