...
$f(x)=\left{x-(a+bi)\right}(x-c)(x-d)$의 상수항은 허수가 되므로 계수가 모두 실수라는 사실에 위배됩니다. 따라서 이러한 경우는 존재할 수 없습니다.

3) 두 근이 복소수이고 한 근은 실수인 경우
$f(x)=\left{x-(a+bi)\right}\left{x-(c+di)\right}(x-e)$의 모든 계수가 실수가 되므로 주어진 조건을 만족합니다.
