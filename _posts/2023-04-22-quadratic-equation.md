...
두 근의 합과 곱 외에 두 의 차 또한 다음과 같이 구할 수 있습니다.
...
이러한 사실을 조금 더 확장해보겠습니다. 만약 같은 $y$값을 가지게 하는 두 수 $x=m$과 $x=n$이 있다면 이 두 수의 평균인 $\frac{m+n}{2}$가 대칭축을 정의하는 $p$값이 된다는 것을 알 수 있습니다. 즉, $f(m)=f(n)$이면 $p=\frac{m+n}{2}$ 됩니다.
