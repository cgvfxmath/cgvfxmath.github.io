...
즉, $D>0$이면 $y=f(x)$의 그래프는 $x$축과 두 점에서 만나고, $D<0$이면 $y=f(x)$의 그래프는 $x$축과 만나지 않습니다. 따라서 다음과 같은 사실이 성립합니다.
* $D<0$이고 $a<0$이면 모든 실수 $x$에 대해서 $f(x)<0$ 입니다. 즉, 이때 $q<0$이 됩니다.
* $D<0$이고 $a>0$이면 모든 실수 $x$에 대해서 $f(x)>0$ 입니다. 즉, 이때 $q>0$이 됩니다.
이러한 사실은 그래프를 그려서 고찰해보면 쉽게 알 수 있습니다.
