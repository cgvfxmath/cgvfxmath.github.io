...
그런데 $(a+b)^n = (a+b)^{n-1} (a+b)$이므로
$(a+b)^n$의 $k+1$번째 항과 $(a+b)^{n-1}의 $k$번째 항에 각각 $b$와 $a$를 곱해서 만들어지는 것을 알 수 있다.

인 $_{n} C _{k} a^{n-k} b^{k}$는
인 $_{n-1} C _{k-1} a^{n-k} b^{k-1}$과
$(a+b)^{n-1}의 $k+1$번째 항인 $_{n-1} C _{k} a^{n-k-1} b^{k}$에
