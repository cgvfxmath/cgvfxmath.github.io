...
3. **주축(principal axis)**이란 어떤 대상이나 시스템의 핵심적인 성질을 나타내는 기하학적인 축을 의미합니다. 서로 직교하는 고유벡터들은 확률변수 $\mathbf{X}$의 특징을 잘 표현하는 주축으로 사용할 수 있습니다. 공분산 행렬 $\mathbf{C}$의 고유값들의 크기가 $\lambda_1 > \lambda_2 > \lambda_3 > \cdots$ 이라면 각 축의 중요도는 $\mathbf{v}_1, \mathbf{v}_2, \mathbf{v}_3, \cdots$ 순서로 설정됩니다.
...
그런데 고유벡터 $\mathbf{v}_i$방향으로의 분산값은 $\lambda_i$와 같으므로 가장 큰 고유값을 가지는 고유벡터는 가장 큰 분산의 방향을 나타내는 것을 알 수 있습니다. 즉, 분산의 전역 최대값(global maximum) 방향는 첫 번째 고유벡터의 방향과 일치합니다.
