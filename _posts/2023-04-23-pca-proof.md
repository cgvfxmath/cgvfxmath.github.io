...
이때 $a$번째 특성과 $b$번째 특성의 **상관관계(correlation)**는 어떻게 정량화할 수 있을까요? $a$번째 특성을 나타내는 벡터는 $\mathbf{f}_a$이고, $b$번째 특성을 나타내는 벡터는 $\mathbf{f}_b$이기 때문에 두 벡터를 내적하면 두 가지 특성 사이의 상관관계를 정량화하여 나타낼 수 있습니다. 왜냐하면 내적은 두 벡터의 방향이 일치하여 두 벡터가 이루는 각이 0일 때 최대값을 반환하고, 두 벡터가 수직하여 상관관계가 전혀 없을 때는 0을 반환하는 연산이기 때문입니다. 다만 표본의 개수에 따라서 결과의 범위가 너무 많이 달라지는 것을 피하기 위해서 두 벡터의 내적을 $m$으로 나누어서 정규화시킨 값을 두 특성 사이의 상관관계 계측값으로 사용합니다.
...
3. $\lambda_1 > \lambda_2 > \lambda_3 > \cdots$ 일 때 큰 분산값을 가지는 방향은 차례대로 $\mathbf{v}_1, \mathbf{v}_2, \mathbf{v}_3, \cdots$ 입니다. 즉, 가장 큰 분산을 가지는 방향은 가장 큰 고유값을 가지는 고유벡터의 방향입니다. 이 방향으로의 축을 첫 번째 **주축(principal axis)**이라고 부릅니다.
...
<p style="text-align: center;">$\mathbf{u} = u_1 \mathbf{v}_1 + u_2 \mathbf{v}_2 + \cdots + u_n \mathbf{v}_n$</p>
...
$= (u_1, u_2, \cdots, u_n) \cdot (u_1 \lambda_1, u_2 \lambda_2, \cdots, u_n \lambda_n)$
...
그런데 고유벡터 $\mathbf{v}_i$방향으로의 분산값은 $\lambda_i$와 같으므로 가장 큰 고유값을 가지는 고유벡터는 가장 큰 분산의 방향을 나타내는 것을 알 수 있습니다.

그리고 그 다음으로 큰 고유값을 가지는 고유벡터는 두 번째로 큰 분산의 방향을 나타냅니다.

그리고 그 다음으로 큰 고유값을 가지는 고유벡터는 세 번째로 큰 분산의 방향을 나타냅니다.

그리고 그 다음으로 큰 고유값을 가지는 고유벡터는 네 번째로 큰 분산의 방향을 나타냅니다.
