...
1. 확률변수 $\mathbf{X}$의 $i$번째 표본을 나타내는 위치벡터 $\mathbf{p}_i$를 $\mathbf{u}$ 방향으로 투영시킨 결과를 $\mathbf{q}_i$라 하고, 원점으로부터 벡터 $\mathbf{q}_i$의 끝점까지의 부호거리(signed distance)를 $q_i$라 하면 방향으로의 분산 $Var_{\mathbf{u}}(\mathbf{X})$는 다음과 같이 계산할 수 있습니다.
...
=
\frac{1}{m} \sum_{i=1}^{m} (\mathbf{p}_i^\text{T} \mathbf{u})^2
=
...
2. 공분산 행렬 $\mathbf{C}$의 $i$번째 고유값과 고유벡터를 각각 $\lambda_i$와 $\mathbf{v}_i$라 하면 다음과 같은 관계식이 성립합니다.
...
3. 가장 큰 분산을 가지는 방향을 구하는 문제는 다음과 같이 기술할 수 있습니다.
