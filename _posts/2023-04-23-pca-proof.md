...
1. 확률변수 $\mathbf{X}$의 $i$번째 표본을 나타내는 위치벡터 $\mathbf{p}_i$ $\mathbf{u}$ 방향으로 투영시킨 결과를 $\mathbf{q}_i$라 하고, 원점으로부터 벡터 $\mathbf{q}_i$의 끝점까지의 부호거리(signed distance)를 $q_i$라 하면 방향으로의 분산 $Var_{\mathbf{u}}(\mathbf{X})$는 다음과 같이 계산할 수 있습니다.
