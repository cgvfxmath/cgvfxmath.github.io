...
위치벡터 $\mathbf{p}_{i}$를 단위벡터 $\mathbf{u}$에 투영한 결과를 $\mathbf{q}_{i}$라 하고, 





원점으로부터 이 벡터의 끝점까지의 부호거리(signed distance)를 ${q}_{i}$라 하면 방향으로의 분산 ${Var}_{\mathbf{u}}(\mathbf{X})$는 다음과 같이 계산할 수 있습니다.
